...
async def test_filter_by_filename(client, db_session):
    ...
    await crud.create_document(db_session, id="d2", filename="recibo.pdf",
        ...
        upload_date=datetime(2025, 3, 2, tzinfo=timezone.utc),
        updated_at=datetime(2025, 3, 2, tzinfo=timezone.utc))
    await db_session.commit()
...
async def test_pagination_total_reflects_filter(client, db_session):
    for i in range(5):
        await crud.create_document(db_session, id=f"d{i}", filename=f"factura_{i}.pdf",
            ...
            upload_date=datetime(2025, 3, i+1, tzinfo=timezone.utc),
            updated_at=datetime(2025, 3, i+1, tzinfo=timezone.utc))
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_sort_by_total_amount_numeric(client, db_session):
    for i, (doc_id, amount) in enumerate([("d1", "200.00"), ("d2", "1200.00")]):
        await crud.create_document(db_session, id=doc_id, filename=f"{doc_id}.pdf",
            format=".pdf", file_path=f"/tmp/{doc_id}.pdf", file_size=100, status="completed",
            upload_date=datetime(2025, 3, i+1, tzinfo=timezone.utc),
            updated_at=datetime(2025, 3, i+1, tzinfo=timezone.utc))
        await crud.create_extraction(db_session, doc_id,
            result={"invoice_number": "001", "issuer_name": "SA", "issuer_cif": "B12345678",
                    "recipient_name": "SL", "recipient_cif": "A87654321",
                    "issue_date": "2025-03-01", "total_amount": amount,
                    "invoice_type": "STANDARD", "status": "valid", "validation_errors": []},
            json_path=f"/tmp/{doc_id}.json")
    await db_session.commit()
    resp = await client.get("/api/documents/?sort_by=total_amount&sort_order=asc")
    amounts = [d["total_amount"] for d in resp.json()["documents"] if d["total_amount"]]
    assert float(amounts[0]) < float(amounts[1])  # 200 before 1200 (not lexicographic)
